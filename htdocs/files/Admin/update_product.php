...
<html lang="en" dir="ltr">
  <head>
    ...
    <title>Tech Store. | Edit Process</title>
  </head>
  <body>
    <div class = "row">
...
if (isset($_POST['update'])) {
  $id = mysqli_real_escape_string($sqli,$_POST['p_id']);
  ...
  $pBrand = mysqli_real_escape_string($sqli,$_POST['p_company']);
  $pImage = mysqli_real_escape_string($sqli,$_POST['p_image']);
}
  ...
  else
  {
    ...
    if(in_array($pBrand,$names))
    {
     ...
     if(!empty($_FILES['p_image']['name']))
     {
       ...
       $query = "UPDATE product_table SET name = '$pName', announced_date = '$date', status = '$pStatus',
                Dimensions = '$pDimens', SIM = '$pSim', Screen_Type = '$pDisType', Screen_Size = '$pScSize',
                OS = '$pOS', Chipset = '$pChipset', CPU = '$pCPU', Card_slot = '$pCslot', Internal_memory ='$pMemory',
                Back_Camera = '$pBackCamera', FrontCamera = '$pFrontCamera', Sensors = '$pSensors',battery_type = '$pBtype',
                Charging = '$pBcharging', Available_models = '$pModels',price = '$pPrice', image = '$imgContent', brand_id = '$id_brand' WHERE product_id = $id;";
        $result = mysqli_query($sqli, $query);
        mysqli_close();
        if ($result) {
          ...
          echo '<h2> SQL Error: '.mysqli_error($sqli).'. Please try after sometime<h2>';
          echo "<a href=\"admin.php\">Go Back</a>";
        }
     }
     else {
      ...
      echo '<h2>Brand Not Found. Please Add new brand first.<h2>';
      echo "<a href=\"admin.php\">Go Back</a>";
    }
  }
?>
